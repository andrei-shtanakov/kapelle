...
- Ветка `<type>/<slug>` → push → `gh pr create`. **Перед `gh pr create` сверить
  ...
  ролей рвёт контур, а не просто выглядит неаккуратно. Разводятся логины
  хранением: у каждого профиля токен лежит в СВОЁМ `hosts.yml`
  (`gh auth login --hostname github.com --insecure-storage`, для ревьюера —
  под `GH_CONFIG_DIR=~/.config/review`), потому что общий keyring ключуется по
  хосту и два аккаунта дерутся за одну ячейку: 2026-09-05/06 коллизия
  сработала дважды, в обе стороны. Цена рецепта — токен на диске открытым
  текстом, поэтому если в keyring остаётся ровно один, пусть это будет
  человеческая учётка: права у неё шире, а бот-токен отзывается дешевле.
  Проверено 2026-09-06: с токенами в собственных `hosts.yml` сверка отдаёт
  разные логины. Порядок резолва внутри `gh` — внешний факт, не наш, и менять
  его нам нечем: единственная гарантия здесь — сама сверка перед действием. **Прямые коммиты в
