...
- Ветка `<type>/<slug>` → push → `gh pr create`. **Перед `gh pr create` сверить
  авторство:** `gh api user --jq .login` → `andrei-shtanakov`. `gh auth status`
  для этой сверки не годится: он печатает активный аккаунт из конфига, а токен
  `gh` достаёт из keyring по ХОСТУ — запись профиля ревьюера перекрывает
  основную, и голый `gh` молча уходит от `ai-prosto` при бодром «Active account:
  andrei-shtanakov». Цена уже заплачена: 2026-09-05 так ушёл PR #62, и
  приёмочное ревью стало невозможным по построению — GitHub отказал
  («Can not approve your own pull request»), автор и ревьюер совпали.
  Автор PR — человеческая учётка, ревьюер и мерж — `ai-prosto`; совпадение
  ролей рвёт контур, а не просто выглядит неаккуратно. Логины разводятся
  хранением: `gh auth login --hostname github.com --insecure-storage` для
  основного аккаунта оставляет профиль ревьюера в keyring. **Прямые коммиты в
  `master` запрещены**, как и локальный мерж ветки в `master` в обход PR.
