...
- [ ] **Cost (токены) на прогон** @id:s4-plus-cost-visibility @epic:airun.kapelle-m3
  — незакрытая половина §9.3 — только стоимость: отчёт уже честно
  показывает `tokens = nil` и причину, но `token_usage/1` остаётся швом
  без реальной инструментовки. Вторая половина §9.3 — `interventions` —
  доставлена в PR #64 (`interventions/3` считает holds/resumes/waivers
  из свидетельства цикла, `nil` вместо нуля при нечитаемом view,
  рендерится в `mix kapelle.product.report`, покрыта тестами) — сюда не
  входит и переделке не подлежит. При закрытии M3 либо доставить
  измерение токенов на реальном адаптере, либо явным решением владельца
  вынести за M3.
