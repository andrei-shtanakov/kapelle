...
вердикт» проверить нечем (находка ревью этого же PR). Он **fail-closed**:
ненулевые `discard`/`failure` у drain, статус цикла, не дошедший до `ready`,
любая ось, не равная `:pass`, и пустой
cost-блок или пропавший waiver-ref **в самом вердикте** роняют прогон ненулевым
кодом — проверено, раняющий прогон отдаёт `exit=1`. Статус проверяется отдельно от осей намеренно: продуктовая ось судит по walk'у
...
сразу после drain оно истечь не может, и застрявший `running` прошёл бы приёмку
зелёным. Печатную поверхность скрипт не сверяет (это была бы вторая копия гейта): рендер блока закреплён
`test/mix/tasks/kapelle_product_report_test.exs`, включая строку
`waivers: 1 (concept-draft://CD-002)` и три состояния `tokens`.
Проверки стоят **после** печати обоих отчётов: на красном прогоне вердикт нужнее
всего, именно он несёт диагностику (`:jobs_discarded`, `:stalled`).

Скрипт лежит вне `elixirc_paths` и вне credo, поэтому `scripts/**/*.exs` добавлен
в `.formatter.exs` — формат остаётся единственным штатным гейтом, который его
видит. Поля, которые скрипт читает у вердикта (`cost.iterations_used`,
`stage_jobs`, `wall_ms`, `interventions.waivers`/`waiver_refs`), уже закреплены
`run_verdict_test.exs`, так что переименование в контракте краснит `mix test`, а
не всплывает на следующей ручной приёмке. Приёмочный
инструмент, который печатает красное и выходит нулём, — зелёный гейт без
гарантии; правило то же, что «fail-closed покрывает сам прибор». Прогон одноразовый и в
...
> **Дополнено 2026-09-06, после приёмки:** вопрос решён в тот же день —
> заведён `airun.kapelle-live-runtime` (ai-orchestrators-workspace#32), оба
> пункта переведены на него (kapelle#72), `airun.kapelle-m3` закрывается
> следующим PR в зонтик — обязательство ведётся чекбоксом
> `todo://ai-orchestrators-workspace/kapelle-epic-migration`, а не этой
> строкой: запись приёмки не трекер. Абзац выше оставлен как есть: это датированная
> запись состояния на момент приёмки, а не текущий срез.
...
`harness=observability_gap` недостижим **на fixture-backed прогонах** — то есть
на всей поверхности M3: гэпов наблюдаемости в срезе не осталось. Ветка при этом
не мертва и достижима намеренно: цикл с адресом агента, который срез не
опознаёт (`provider:gpt-5` и любая живая схема будущего #50), без token usage
даёт `:cost_not_instrumented`/`:gap` и красит ось — это и есть fail-closed,
который не даст добавить адаптер без инструментовки молча
(`run_verdict_test.exs`, «an agent address this slice does not know owes a token
figure»).
