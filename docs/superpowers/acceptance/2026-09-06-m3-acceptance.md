...
ненулевые `discard`/`failure` у drain, любая ось, не равная `:pass`, и потеря
самой видимости cost/interventions (пустой cost-блок, пропавший waiver-ref)
роняют прогон ненулевым кодом — проверено, раняющий прогон отдаёт `exit=1`.
