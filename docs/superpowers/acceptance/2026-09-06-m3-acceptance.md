...
ненулевые `discard`/`failure` у drain, любая ось, не равная `:pass`, и пустой
cost-блок или пропавший waiver-ref **в самом вердикте** роняют прогон ненулевым
кодом — проверено, раняющий прогон отдаёт `exit=1`. Печатную поверхность скрипт
не сверяет (это была бы вторая копия гейта): рендер блока закреплён
