...
**Checklist:**
- [x] тест бежит обычным `mix test` без env-переменных, сети и producer-checkout (BEH-16, BEH-17)
- [x] после отрицательного прогона рабочее дерево побайтно не изменено (BEH-17)
- [x] свежесгенерированный `scripts/gen_golden.sh` manifest проходит без ручного преобразования (BEH-18)
- [x] согласованная правка payload+digest проходит проверку — граница задокументирована в тесте как ожидание, не как дыра (BEH-19)
- [x] полный сьют kapelle зелёный; `mix precommit` включает новую проверку
