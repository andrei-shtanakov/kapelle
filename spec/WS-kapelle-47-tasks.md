...
**Checklist:**
- [ ] `/absolute`, `../outside`, `./dir/../../outside`, `./PROVENANCE` — отклоняются до чтения содержимого с классом `unsafe_path` (BEH-06)
- [ ] неканонический алиас (`./workspace/./x`) не даёт вторую запись того же payload (BEH-07)
- [ ] symlink внутри и наружу сценария не хешируется и отклоняется (BEH-08)
- [ ] необъявленный payload / отсутствующий payload / дубликат записи — три различимых класса (BEH-09—BEH-11)
- [ ] вложенные файлы (`workspace/…` и глубже) входят в покрытие — тест на глубину ≥2 (BEH-12)
...
**Checklist:**
- [ ] правка одного байта payload при неизменном manifest — стабильный `checksum_mismatch` (BEH-13); красный воспроизведён и руками на временной копии (приёмка kapelle#47)
- [ ] семантически эквивалентный, но байтово иной JSON — тоже mismatch (BEH-14)
- [ ] два независимых нарушения в разных сценариях перечислены оба, порядок детерминирован (BEH-15)
