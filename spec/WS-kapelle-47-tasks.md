...
> **Сверка статусов 2026-09-06** (`todo://kapelle/tasks-md-reconciliation`):
> статусы ниже сверены с merge-историей репозитория. Все четыре задачи
> доставлены на `master`, ни один статус не менялся; строка `**Доставлено:**`
> под статусом называет PR и merge-коммит.
>
> Блок сверки и строки `**Доставлено:**` внесены **руками** 2026-09-06, уже
> после генерации файла task_bridge: регенерация из behaviour-бандла
> WS-kapelle-47 их затрёт, если не перенести. Тот же маппинг продублирован в
> закрытом пункте `todo://kapelle/tasks-md-reconciliation` и в PR #70 — так
> что восстановить его есть откуда.

### TASK-001: Каркас проверки — discovery сценариев и fail-closed разбор PROVENANCE
P1 | ✅ DONE   Est: 1d

**Доставлено:** PR #55 (`e3c1c0f`); behaviour-бандл — PR #51 (`5ea5b1b`), спека-черновик — PR #53 (`8ab656a`).
...
**Доставлено:** PR #57 (`9f7202f`); правка форматирования TOCTOU-регрессии — PR #58 (`73a281f`).
