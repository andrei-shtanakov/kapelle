defmodule Kapelle.Orchestrator.Records.RunTaskTest do
  ...
  test "changeset/2 is valid with run_id, decision_id, task_id, and status", %{
    run: run,
    decision: decision
  } do
    changeset =
  ...
  test "changeset/2 requires run_id, decision_id, task_id, and status" do
  ...
  test "changeset/2 defaults artifacts to an empty list", %{run: run, decision: decision} do
    changeset =
      RunTask.changeset(%RunTask{}, %{
        run_id: run.id,
        decision_id: decision.id,
  ...
  test "changeset/2 casts output, duration_ms, and artifacts", %{run: run, decision: decision} do
    attrs = %{
      run_id: run.id,
      decision_id: decision.id,
      task_id: "task-1",
  ...
  test "changeset/2 persists and reloads a run_task linked to its run and decision", %{
    run: run,
    decision: decision
  } do
    {:ok, run_task} =
    ...
    assert %RunTask{run_id: run_id, decision_id: decision_id} = Repo.get!(RunTask, run_task.id)
    assert run_id == run.id
    assert decision_id == decision.id
  end

  test "changeset/2 persists and reloads artifacts as a list of maps", %{
    run: run,
    decision: decision
  } do
    {:ok, run_task} =
  ...
  test "changeset/2 surfaces a foreign_key_constraint on a non-existent run_id", %{
    decision: decision
  } do
    changeset =
      RunTask.changeset(%RunTask{}, %{
  ...
  test "changeset/2 surfaces a foreign_key_constraint on a non-existent decision_id", %{
    run: run
  } do
    changeset =
      RunTask.changeset(%RunTask{}, %{
  ...
  test "changeset/2 surfaces a unique_constraint on decision_id", %{run: run, decision: decision} do
    {:ok, _run_task} =
      Repo.insert(
        RunTask.changeset(%RunTask{}, %{
          run_id: run.id,
